...
export default function MyOrdersPage() {
  ...
  const handleConfirmAction = async () => {
    ...
    try {
        // Send 'action' param to API to distinguish between cancel and delete
  ...
  return (
    <div className="min-h-screen bg-[#fff0f5]">
      ...
      {/* --- CONFIRMATION DIALOG (REPLACES ALERT) --- */}
      {confirmDialog.show && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4 animate-fade-in">
            <div className="bg-white p-6 rounded-[1.5rem] shadow-2xl max-w-sm w-full text-center border border-white/60">
                <h3 className="text-xl font-bold text-[#4A1D46] mb-2">{confirmDialog.title}</h3>
                <p className="text-gray-600 mb-6 text-sm font-medium">{confirmDialog.message}</p>
                <div className="flex gap-3 justify-center">
                    ...
                    <button 
                        ...
                        className={`px-5 py-2 text-white rounded-lg text-sm font-bold transition shadow-md hover:scale-105 ${
                            confirmDialog.type === 'delete' 
                            ? 'bg-gradient-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700' 
                            : 'bg-gradient-to-r from-[#880e4f] to-[#ad1457]'
                        }`}
                    >
