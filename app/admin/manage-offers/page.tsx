...
function ProductListContent() {
  ...
  // --- NEW: FETCH REVIEWS ON DEMAND WHEN MODAL OPENS ---
  const openPreviewModal = async (item: any) => {
      const displayImg = (item.images && item.images.length > 0) ? item.images[0] : item.image;
      const targetId = String(item.id || item.productid || item.itemid).toLowerCase();
      
      setPreviewActiveImage(displayImg);
      // Open modal immediately with loading state
      setPreviewProduct({ ...item, actualReviews: null, isLoadingReviews: true });

      try {
          let allReviews = [];
          // Try multiple common endpoints where reviews might live
          const endpoints = ['/api/product_reviews', '/api/reviews', '/api/admin/reviews'];
          ...
          // Filter reviews specifically for this product based on your exact DB columns
          const filteredReviews = allReviews.filter((r: any) => {
              ...
              const matchId = (dbProductId === targetId) || (dbItemId === targetId);
              const isNotHidden = r.is_hidden == 0 || r.is_hidden === false || r.is_hidden == null;
              return matchId && isNotHidden;
          });

          setPreviewProduct({ ...item, actualReviews: filteredReviews, isLoadingReviews: false });
      } catch (error) {
          console.error("Failed to fetch reviews:", error);
          setPreviewProduct({ ...item, actualReviews: [], isLoadingReviews: false });
      }
  };
              ...
              >
                  ...
                  <div className="md:w-1/2 p-8 overflow-y-auto custom-scrollbar flex flex-col bg-gradient-to-b from-[#2E1029] to-[#4A1D46]">
                      ...
                      <div className="flex items-center gap-1 mb-4">
                          {previewProduct.isLoadingReviews ? (
                              <span className="text-xs text-gray-400 animate-pulse">Loading reviews...</span>
                          ) : (
                              <>
                      ...
                      <div className="mt-auto pt-6 border-t border-white/10">
                          ...
                          {previewProduct.isLoadingReviews ? (
                              <p className="text-gray-400 text-sm italic bg-white/5 p-4 rounded-xl text-center animate-pulse">Loading...</p>
                          ) : (!previewProduct.actualReviews || previewProduct.actualReviews.length === 0) ? (
                              ...
                              <div className="flex flex-col gap-4">
                                  {previewProduct.actualReviews.map((review: any, idx: number) => {
                                      const reviewerName = review.customer_name || review.customerName || review.author || review.userName || review.name || "Verified Customer";
                                      const reviewText = review.comment || review.text || review.content || review.review_text || review.reviewText;
